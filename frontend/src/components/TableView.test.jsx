...
import TableView, { loadMaskedColumns, saveMaskedColumns } from './TableView'
...
describe('TableView Field Masking', () => {
  ...
  describe('header context menu', () => {
    ...
    it('toggles mask state when clicking mask option', () => {
      ...
      expect(localStorageMock.setItem).toHaveBeenCalled()
      const savedData = JSON.parse(localStorageMock.setItem.mock.calls.find(c => c[0] === 'mongopal-masked-columns')[1])
  ...
  describe('unmask all columns', () => {
    ...
    it('clears all masked columns when clicking "Unmask All"', () => {
      ...
      const setItemCalls = localStorageMock.setItem.mock.calls.filter(c => c[0] === 'mongopal-masked-columns')
