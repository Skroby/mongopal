...
let capturedProps: MonacoDiffEditorProps = {} as MonacoDiffEditorProps

vi.mock('./MonacoDiffEditor', () => ({
  default: (props: MonacoDiffEditorProps) => {
    capturedProps = props
    return (
...
beforeEach(() => {
  capturedProps = {} as MonacoDiffEditorProps
})
...
describe('DocumentDiffView', () => {
  ...
  describe('property sorting', () => {
    it('sorts object keys alphabetically for consistent comparison', () => {
      ...
      const originalKeyOrder = original.match(/"(\w+)":/g)?.map((k: string) => k.replace(/[":]/g, '')) || []
      const modifiedKeyOrder = modified.match(/"(\w+)":/g)?.map((k: string) => k.replace(/[":]/g, '')) || []
