...
import { useConnection, ConnectionContextValue } from './contexts/ConnectionContext'
import IndexView, { MongoIndex } from './IndexView'
...
// Type for mocked useConnection
const mockedUseConnection = useConnection as Mock

// Mock the go object with dynamic implementation
let mockListIndexes: Mock
let mockCreateIndex: Mock
let mockDropIndex: Mock

interface DefaultConnectionContext {
  activeConnections: string[]
  connectingIds: Set<string>
  connect: Mock
}

const defaultConnectionContext: DefaultConnectionContext = {
...
beforeEach(() => {
  ...
  mockDropIndex = vi.fn()
  // Use window.go from the test setup and override specific methods
  if (window.go?.main?.App) {
    window.go.main.App.ListIndexes = mockListIndexes
    window.go.main.App.CreateIndex = mockCreateIndex
    window.go.main.App.DropIndex = mockDropIndex
  }
  mockedUseConnection.mockReturnValue(defaultConnectionContext as unknown as ConnectionContextValue)
})
...
describe('IndexView', () => {
  ...
  describe('not connected state', () => {
    it('displays connect button when not connected', () => {
      mockedUseConnection.mockReturnValue({
        ...
        activeConnections: [],
      } as unknown as ConnectionContextValue)
      renderWithProvider(<IndexView {...defaultProps} />)
  ...
  describe('delete index', () => {
    ...
    it('shows confirmation dialog when delete button is clicked', async () => {
      ...
      expect(
        screen.getByText(/Are you sure you want to drop the index "email_1"/)
      ).toBeInTheDocument()
    })
  })
