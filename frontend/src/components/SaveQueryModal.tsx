...
import { SavedQuery, SavedQueryInput } from '../types/wails.d'

const go = window.go?.main?.App

// Re-export SavedQuery for backward compatibility
export type { SavedQuery }

/**
 * Props for the existing query when editing.
 */
...
/**
 * Props for the SaveQueryModal component.
 */
export interface SaveQueryModalProps {
  ...
  onClose: () => void
  /** Connection ID for the query context */
  connectionId: string
  /** Database name for the query context */
  database: string
  /** Collection name for the query context */
  collection: string
  /** The query string to save */
  query: string
  /** If provided, we're editing an existing query */
  existingQuery?: ExistingQuery | null
  /** Callback when the query is saved */
...
export default function SaveQueryModal({
  ...
  existingQuery = null,
  onSaved,
}: SaveQueryModalProps): React.ReactElement | null {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    setSaving(true)
    try {
      const queryData: SavedQueryInput = {
        id: existingQuery?.id,
        name: name.trim(),
      ...
      notify.error(error?.message || 'Failed to save query')
    } finally {
      setSaving(false)
    }
  }
