...
import { OperationProvider, useOperation, OperationContextValue } from './OperationContext'

// Test component that uses the context
interface TestConsumerProps {
  onMount?: (context: OperationContextValue) => void
}
...
describe('OperationContext', () => {
  ...
  describe('startOperation', () => {
    it('should add an operation', () => {
      let contextRef: OperationContextValue | undefined
      render(
        <OperationProvider>
  ...
  describe('beforeunload handler', () => {
    ...
    let registeredHandler: ((event: BeforeUnloadEvent) => void) | null

    beforeEach(() => {
      registeredHandler = null
      addEventListenerSpy = vi.spyOn(window, 'addEventListener').mockImplementation((event, handler) => {
        if (event === 'beforeunload') {
          registeredHandler = handler as (event: BeforeUnloadEvent) => void
        }
      })
    ...
    it('should prevent unload during destructive operation', () => {
      ...
      const event = { preventDefault: vi.fn() } as unknown as BeforeUnloadEvent & { returnValue: string }
