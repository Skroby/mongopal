...
import { SchemaProvider, useSchema, SchemaResult, SchemaContextValue } from './SchemaContext'
...
beforeEach(() => {
  ...
  if (window.go?.main?.App) {
    window.go.main.App.InferCollectionSchema = mockInferCollectionSchema
  }
})
...
const wrapper = ({ children }: WrapperProps) => <SchemaProvider>{children}</SchemaProvider>
...
describe('SchemaContext', () => {
  ...
  describe('fetchSchema', () => {
    it('fetches and caches schema', async () => {
      ...
      let fetchResult: Awaited<ReturnType<SchemaContextValue['fetchSchema']>> = null
      await act(async () => {
  ...
  describe('isSchemaLoading', () => {
    it('returns true while schema is loading', async () => {
      let resolvePromise: (value: SchemaResult) => void
      mockInferCollectionSchema.mockImplementation(() => new Promise<SchemaResult>(resolve => {
      ...
      // Resolve the promise
      await act(async () => {
        resolvePromise!({ collection: 'testColl', sampleSize: 10, totalDocs: 100, fields: {} })
      })
