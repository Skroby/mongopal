 ...
 * @param {string} str - The mongosh output string
 * @returns {string} - String with type wrappers removed
 */
export function stripResultTypeWrappers(str) {
 ...
 */
export function convertMongoshTypes(str) {
  let result = str
  ...
  result = result.replace(
    ...
    (_, subType, base64) => `{"$binary":{"base64":"${base64}","subType":"${parseInt(subType).toString(16).padStart(2, '0')}"}}`
  )
  ...
  result = result.replace(
    /\/([^/]+)\/([gimsuy]*)/g,
    (match, pattern, flags) => `{"$regularExpression":{"pattern":"${pattern.replace(/"/g, '\\"')}","options":"${flags}"}}`
  )
 ...
 * @param {string} str - String with potential single-quoted values
 * @returns {string} - String with double-quoted values
 */
export function convertSingleQuotesToDouble(str) {
 ...
 * @param {string} str - The string with potentially unquoted keys
 * @returns {string} - String with quoted keys
 */
export function quoteUnquotedKeys(str) {
 ...
 * @param {string} str - JSON-like string with potential trailing commas
 ...
 * @param {string} output - Raw mongosh output
 * @returns {{ success: boolean, data: any[], error?: string }}
 */
export function parseMongoshOutput(output) {
  ...
  try {
    ...
    return {
      success: false,
      data: [],
      error: `Failed to parse mongosh output: ${e.message}`
    }
  }
}
