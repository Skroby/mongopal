...
describe('parseMongoshOutput', () => {
  it('returns empty array for empty input', () => {
    const result = parseMongoshOutput('')
    expect(result.success).toBe(true)
    expect(result.data).toEqual([])
  })

  it('returns error for null input', () => {
    const result = parseMongoshOutput(null)
  ...
  it('parses valid JSON array', () => {
    const input = '[{"name": "John"}, {"name": "Jane"}]'
    const result = parseMongoshOutput(input)
    expect(result.success).toBe(true)
  ...
  it('parses mongosh format with ISODate', () => {
    ...
    const result = parseMongoshOutput(input)
    expect(result.success).toBe(true)
    expect(result.data[0].createdAt).toEqual({ $date: '2023-01-15T10:30:00.000Z' })
  })
  ...
  it('parses mongosh format with NumberLong', () => {
    ...
    const result = parseMongoshOutput(input)
    expect(result.success).toBe(true)
    expect(result.data[0].count).toEqual({ $numberLong: '9223372036854775807' })
  })
  ...
  it('parses complex mongosh aggregation result', () => {
    ...
    expect(result.data[0].count).toEqual({ $numberLong: '10' })
  ...
  it('parses mongosh format with nested objects', () => {
    ...
    expect(result.data[0].user.address.city).toBe('NYC')
  })
  ...
  it('parses mongosh format with arrays', () => {
    ...
    const result = parseMongoshOutput(input)
    expect(result.success).toBe(true)
    expect(result.data[0].tags).toEqual(['a', 'b', 'c'])
    expect(result.data[0].scores).toEqual([1, 2, 3])
  })
  ...
  it('parses mongosh format with mixed types', () => {
    ...
    expect(result.data[0].count).toEqual({ $numberLong: '100' })
    expect(result.data[0].ratio).toBe(0.5)
  ...
  it('parses realistic aggregation output', () => {
    ...
    expect(result.data[0].totalSales).toEqual({ $numberLong: '15000' })
    expect(result.data[0].avgPrice).toBe(299.99)
    expect(result.data[0].topProducts).toHaveLength(2)
  })
  ...
  it('parses mongosh output with single-quoted strings', () => {
    ...
    expect(result.success).toBe(true)
    expect(result.data).toHaveLength(2)
    expect(result.data[0].count).toBe(262152)
    expect(result.data[0].tokenType).toBe('PAT')
    expect(result.data[0].ownerType).toBe('User')
    expect(result.data[0].oldestToken).toEqual({ $date: '2023-08-18T18:28:14.838Z' })
    expect(result.data[1].tokenType).toBe('M2M')
    expect(result.data[1].ownerType).toBe('Machine2Machine')
  })
  ...
  deleted: DeleteResult {
    ...
    expect(result.data[0].inserted.insertedIds['0']).toEqual({ $oid: '697d75381772b9e2e58089a9' })
    expect(result.data[0].deleted.acknowledged).toBe(true)
    expect(result.data[0].deleted.deletedCount).toBe(6)
  })
  ...
  insertedId: ObjectId('697d75381772b9e2e58089a9')
    ...
    expect(result.data[0].insertedId).toEqual({ $oid: '697d75381772b9e2e58089a9' })
  })
  ...
  upsertedCount: 0
    ...
    expect(result.success).toBe(true)
    expect(result.data[0].acknowledged).toBe(true)
    expect(result.data[0].matchedCount).toBe(5)
    expect(result.data[0].modifiedCount).toBe(3)
  })
})
