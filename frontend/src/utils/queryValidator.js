 ...
 */
function relaxedJsonToStrict(text) {
  ...
  result = result.replace(/([{,]\s*)([a-zA-Z_][a-zA-Z0-9_]*)(\s*:)/g, (match, prefix, key, suffix) => {
 ...
 */
function extractBalancedContent(text, startChar, endChar) {
  if (!text.startsWith(startChar) && !text.trim().startsWith(startChar)) {
 ...
 */
function extractJsonFromQuery(query) {
 ...
 */
function checkCommonMistakes(text) {
  const diagnostics = []
 ...
 * @param {Array<Object>} diagnostics - Array of diagnostic objects from validateQuery
 * @returns {Array<Object>} Array of Monaco marker objects
 */
