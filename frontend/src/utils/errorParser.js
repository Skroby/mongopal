 ...
 * Each pattern has:
 * - pattern: RegExp or string to match against the error message
 ...
 * @returns {Object} Parsed error info with:
    ...
    for (const pattern of errorDef.patterns) {
      if (pattern.test(rawMessage)) {
        return {
          ...
          actionData: errorDef.actionData,
          isKnown: true,
        }
      }
    }
 ...
 * @returns {boolean}
 */
export function errorHasAction(error, actionType) {
 ...
 *
 * @param {string|Error} error - The error message or Error object
 * @param {number} maxLength - Maximum length for the summary (default: 100)
 * @returns {string}
 */
export function getErrorSummary(error, maxLength = 100) {
  const parsed = parseError(error)
